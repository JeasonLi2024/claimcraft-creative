...
项目已完成 T0（核心链路）→ T1（产品闭环）→ T2（工程化）→ T3（LLM 工作流 + RAG + SSE 流式）→ v10（场景泛化 + 三阶段标准 RAG）五阶段迭代，后端基于 **LangGraph 7 节点工作流 + EventDepot 事件保留站** 实现 SSE 流式推送，前端基于 **React 19 + TypeScript + Golden Time 设计系统** 实时展示工作流中间产物与 complaint token 逐字流，提供 **44 条 RESTful + SSE API**，支持 Docker 一键部署。
...
| 账户中心 | Django `auth.User` + `UserProfile` / `UserPreference` / `UserSession` / `EmailVerificationChallenge` |
| 邮件服务 | Agent Mail CLI（OAuth）优先 + QQ SMTP 自动兜底 |
...
│   │   ├── services/                    # 业务服务（OCR + 抽取 + 时间线 + 投诉 + RAG + 法律工具 + 账户中心）
...
│   │   │   ├── avatar_service.py        # 头像原图/展示图处理与清理
│   │   │   ├── mail_service.py          # 邮件适配层（Agent Mail CLI + QQ SMTP）
...
│   │   │   ├── ProfilePage.tsx                    # 个人信息页（头像 / 偏好 / 邮箱验证 / 会话）
...
│   ├── backend-user-auth-optimization-design.md  # 用户体系与 JWT 认证后端优化设计
│   ├── server-mail-service-deployment-guide.md   # 服务器邮件服务部署说明
│   └── superpowers/specs/
...
| `/profile` | ProfilePage（账户中心） | AuthGuard |
...
| GET / PATCH | `/api/auth/me/` | 当前用户聚合信息 / 更新资料 |
| POST / DELETE | `/api/auth/me/avatar/` | 上传头像 / 删除头像 |
| POST | `/api/auth/me/email/send-code/` | 向当前邮箱发送验证码 |
| POST | `/api/auth/me/email/verify/` | 校验当前邮箱验证码 |
| POST | `/api/auth/me/email/change/request/` | 向新邮箱发送验证码 |
| POST | `/api/auth/me/email/change/confirm/` | 确认新邮箱变更 |
| GET / PATCH | `/api/auth/me/preferences/` | 读取 / 更新偏好 |
...
| POST | `/api/cases/<id>/apply-preset/` | 应用模板预设 |
| GET | `/api/stats/dashboard/` | 7 项仪表盘统计 |
...
| `CLAIMCRAFT_MAIL_PROVIDER_ORDER` | agent_cli,smtp | 邮件发送优先级 |
| `CLAIMCRAFT_AGENT_MAIL_COMMAND` | `/usr/local/bin/agently-cli` | Agent Mail CLI 路径 |
| `CLAIMCRAFT_AGENT_MAIL_HOME` | `/srv/claimcraft/.agently-home` | CLI OAuth 凭据目录 |
| `CLAIMCRAFT_SMTP_HOST` | smtp.qq.com | QQ SMTP 兜底主机 |
...
#   CLAIMCRAFT_AGENT_MAIL_HOME=...      # Agent Mail CLI OAuth 凭据目录
#   CLAIMCRAFT_SMTP_PASSWORD=...        # QQ 邮箱授权码（兜底通道）
...
### 2.1 Agent Mail CLI（可选但推荐）

如果需要启用账户中心里的真实邮箱验证码能力，建议本地额外完成以下步骤：
...
项目运行时会优先调用 `Agent Mail CLI` 发送验证邮件，失败后自动回退 `QQ SMTP`。  
服务器部署详见 [docs/server-mail-service-deployment-guide.md](docs/server-mail-service-deployment-guide.md)。
