...
项目已完成 T0（核心链路）→ T1（产品闭环）→ T2（工程化）→ T3（LLM 工作流 + RAG + SSE 流式）四阶段迭代，后端基于 **LangGraph 7 节点工作流 + EventDepot 事件保留站** 实现 SSE 流式推送，前端基于 **React 19 + TypeScript + Golden Time 设计系统** 实时展示工作流中间产物与 complaint token 逐字流，提供 30 条 RESTful + SSE API，支持 Docker 一键部署。
...
| RAG 知识库 | pgvector + Qwen3-VL-Embedding-8B（法律条文向量检索） |
...
# （可选）导入法律条文 RAG 知识库（15 条法律条文 + 10 个判例）
