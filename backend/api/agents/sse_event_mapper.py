...
参考 spec 第 5.3 节。负责：
- 识别工作流 7 节点的 on_chain_start / on_chain_end 事件
- 跟踪 current_node 用于 complaint_node 的 token 流过滤
- 计算 duration_ms（节点开始到结束的耗时）
- 从节点输出提取对应 state 字段作为产物
- 仅 complaint_node 的 on_chat_model_stream 映射为 complaint.token
...
class SSEEventMapper:
    ...
    # 工作流 7 节点名称集合（与 graph.py 中 add_node 的名称一致）
    NODE_NAMES: set[str] = {
        "preclassify",
        "ocr",
        "classify",
    ...
    async def map(self, raw_event: dict) -> list[SSEEvent]:
        ...
        （complaint 节点额外产生 complaint.done）。
        ...
        # 3. LLM token 流：仅 complaint_node 的 on_chat_model_stream 映射为 complaint.token
    ...
    def _build_node_complete(self, name: str, output: dict) -> list[SSEEvent]:
        """构建 node.complete 事件（complaint 节点额外加 complaint.done）。"""
        ...
        # complaint 节点完成额外产生 complaint.done
        if name == "complaint":
