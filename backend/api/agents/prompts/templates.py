...
4. 引用法条格式：根据《法律名称》第X条规定，...
5. 若工具查询结果为空，不得引用具体法条，仅做事实陈述"""


# ===== Tools 启用时的 prompt 片段（v10 新增）=====
TOOLS_ENABLED_SECTION = """可用工具（按需调用，提升专业性）：
- lookup_law(keyword, category)：查询法律条文（RAG 向量检索）
- lookup_precedent(scenario)：查询类似维权判例
- lookup_platform_rule(platform, issue)：查询平台投诉规则
- calculate_compensation(violation_type, amount)：计算法定赔偿金额

调用建议：
- 涉及欺诈/食品安全/质量问题→必查 calculate_compensation
- 需要法律依据支撑→查 lookup_law
- 需要参考类似案例→查 lookup_precedent
...
# ===== 证据链构造 prompt（v10 增强版：注入 RAG 检索的法条）=====
...
{
  "evidence_category": "chat_screenshot|product_order|logistics_tracking|payment_record|invoice|other",
...
分类说明：
- chat_screenshot：聊天截图（即时通讯对话、客服沟通记录）
- product_order：商品订单（订单详情页、商品购买页）
- logistics_tracking：物流跟踪（物流信息、快递追踪）
- payment_record：支付凭证（付款截图、转账记录）
- invoice：发票（增值税普通/专用发票、电子发票）
- other：无法归入以上类别
