...
def build_physical_note_section(is_physical_evidence: bool, physical_note: str) -> str:
    """构造物证说明 prompt 片段（preclassify_node 调用）。

    - 物证且说明非空：返回引导描述段
    - 否则：返回空字符串
    """
    if not is_physical_evidence or not physical_note:
        return ""

    # 截断到 300 字，避免 prompt 膨胀
    note = physical_note.strip()[:300]
    return (
        ...
        'evidence_category 应选 work_record / other / communication_record 等物证类，'
        '不要选需要文字的类别（如 chat_screenshot / product_order）。'
    )
