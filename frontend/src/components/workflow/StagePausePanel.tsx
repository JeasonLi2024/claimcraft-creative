...
export function StagePausePanel({ caseId }: { caseId: number }) {
  ...
  const errorId = "stage-pause-error"
  const hasError = Boolean(localError || storeError)
  const ariaErrorProps: { "aria-invalid"?: boolean; "aria-describedby"?: string } = hasError
    ...
    <section className="overflow-hidden rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-white shadow-sm">
      <header className="border-b border-amber-200 px-5 py-4">
        <div className="flex items-start gap-3">
          <div className="rounded-xl bg-amber-100 p-2 text-amber-700"><AlertTriangle className="h-5 w-5" aria-hidden="true" /></div>
          <div>
          ...
          <>
              ...
              <div key={row.id} className="rounded-xl border border-slate-200 bg-white p-4">
                ...
                <div className="space-y-3">{scope.evidences!.map((field) => (
                  <label key={field} className="block text-sm font-medium text-slate-700">{FIELD_LABELS[field] || field}
                    {field === "extracted_text" || field === "ocr_summary" ? <textarea maxLength={LIMITS[field]} value={String(row[field as keyof typeof row] ?? "")} onChange={(e) => updateList("evidences", index, field, e.target.value)} {...ariaErrorProps} className={`${textInputClass(true)} mt-1.5`} /> : <input maxLength={LIMITS[field]} value={String(row[field as keyof typeof row] ?? "")} onChange={(e) => updateList("evidences", index, field, e.target.value)} {...ariaErrorProps} className={`${textInputClass()} mt-1.5`} />}
                  </label>
                ))}</div>
              </div>
            ))}
            ...
            {scope.extracted_fields && (products.extracted_fields || []).map((row, index) => (
              <div key={row.id} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 md:grid-cols-2">
                ...
                {scope.extracted_fields!.map((field) => <label key={field} className="text-sm font-medium text-slate-700">{FIELD_LABELS[field] || field}<input maxLength={LIMITS[field]} value={String(row[field as keyof typeof row] ?? "")} onChange={(e) => updateList("extracted_fields", index, field, e.target.value)} {...ariaErrorProps} className={`${textInputClass()} mt-1.5`} /></label>)}
              </div>
            ))}
            ...
            {scope.timeline_nodes && (products.timeline_nodes || []).map((row, index) => (
              <div key={row.id} className="rounded-xl border border-slate-200 bg-white p-4">
                ...
                {scope.timeline_nodes!.map((field) => <label key={field} className="text-sm font-medium text-slate-700">{FIELD_LABELS[field] || field}<textarea maxLength={LIMITS[field]} value={String(row[field as keyof typeof row] ?? "")} onChange={(e) => updateList("timeline_nodes", index, field, e.target.value)} {...ariaErrorProps} className={`${textInputClass(true)} mt-1.5`} /></label>)}
              </div>
            ))}

            {scope.document && products.document && <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">{scope.document.map((field) => <label key={field} className="block text-sm font-medium text-slate-700">{FIELD_LABELS[field] || field}{field === "content" ? <textarea maxLength={LIMITS[field]} value={String(products.document?.[field as keyof typeof products.document] ?? "")} onChange={(e) => updateDocument(field, e.target.value)} {...ariaErrorProps} className={`${textInputClass(true)} mt-1.5 min-h-64`} /> : <input maxLength={LIMITS[field]} value={String(products.document?.[field as keyof typeof products.document] ?? "")} onChange={(e) => updateDocument(field, e.target.value)} {...ariaErrorProps} className={`${textInputClass()} mt-1.5`} />}</label>)}</div>}
          </>
        )}

        {(localError || storeError) && <div id={errorId} role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{localError || storeError}</div>}

        <div className="flex flex-wrap justify-end gap-3 border-t border-amber-100 pt-4">
          <button type="button" onClick={handleCancel} disabled={loading || submitting !== null} className="inline-flex min-h-[44px] items-center gap-2 rounded-lg border border-red-200 bg-white px-4 py-2 text-sm font-medium text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50">{submitting === "cancel" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Square className="h-4 w-4" aria-hidden="true" />}取消工作流</button>
          <button type="button" onClick={handleResume} disabled={loading || submitting !== null} className="inline-flex min-h-[44px] items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50">{submitting === "resume" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" aria-hidden="true" />}保存并继续</button>
        </div>
      </div>
    </section>
  )
