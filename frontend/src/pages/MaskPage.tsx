...
import {
  Eye, EyeOff, Images, LockKeyhole, ScanFace, Shield,
    ...
    <div className="space-y-5 pb-8">
      ...
      <section className="overflow-hidden rounded-[24px] border border-border bg-card shadow-[0_12px_36px_rgba(31,45,38,.05)]">
        <div className="flex flex-wrap items-end justify-between gap-3 border-b border-border p-5 sm:p-6">
          ...
          <span className="rounded-full bg-muted px-3 py-1.5 text-xs text-muted-foreground">{masked ? "脱敏结果" : "原始内容"}</span>
        </div>
        {maskResults.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-muted/40"><th className="px-5 py-3 text-left font-medium text-muted-foreground">证据编号</th><th className="px-5 py-3 text-left font-medium text-muted-foreground">信息类型</th><th className="px-5 py-3 text-left font-medium text-muted-foreground">{masked ? "打码后" : "原文"}</th></tr></thead>
              <tbody>{maskResults.map((result, index) => <tr key={`${result.evidence_code}-${index}`} className="border-t border-border"><td className="px-5 py-3"><span className="rounded-lg bg-[#17231d] px-2.5 py-1 text-xs font-bold text-white">{result.evidence_code}</span></td><td className="px-5 py-3"><PillTag label={TYPE_STYLES[result.type]?.label || "未知"} variant={TYPE_STYLES[result.type]?.variant || "default"} /></td><td className="px-5 py-3 font-mono text-xs text-muted-foreground">{masked ? result.masked : result.original}</td></tr>)}</tbody>
            </table>
          </div>
      ...
      <section className="rounded-[24px] border border-border bg-card p-5 shadow-[0_12px_36px_rgba(31,45,38,.05)] sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          {imageEvidences.length > 0 && <button onClick={handleMaskAllImages} disabled={maskingImages} className="inline-flex items-center gap-2 rounded-xl bg-[#17231d] px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50"><Shield className={cn("h-4 w-4", maskingImages && "animate-pulse")} />{maskingImages ? "处理中..." : "一键打码所有图片"}</button>}
        </div>
        {imageEvidences.length > 0 ? <div className="mt-5 grid gap-5 lg:grid-cols-2">{imageEvidences.map((evidence) => <article key={evidence.id} className="overflow-hidden rounded-2xl border border-border"><div className="flex items-center justify-between bg-muted/45 px-4 py-3"><span className="text-xs font-bold text-secondary">{evidence.code}</span><PillTag label={evidence.mask_status === "done" ? "已打码" : evidence.mask_status === "pending" ? "处理中" : "待处理"} variant={evidence.mask_status === "done" ? "success" : evidence.mask_status === "pending" ? "warning" : "default"} /></div><div className="grid grid-cols-2 gap-px bg-border"><button className="relative bg-white text-left" onClick={() => evidence.image && setLightboxSrc(evidence.image)}><img src={evidence.image || ""} alt={`${evidence.code} 原图`} className="h-44 w-full object-cover" /><span className="absolute bottom-2 left-2 rounded-md bg-black/55 px-2 py-1 text-[10px] text-white">原图</span></button><button className="relative bg-white text-left" onClick={() => setLightboxSrc(evidence.masked_image || evidence.image)}><img src={evidence.masked_image || evidence.image || ""} alt={`${evidence.code} 打码后`} className="h-44 w-full object-cover" /><span className="absolute bottom-2 left-2 rounded-md bg-black/55 px-2 py-1 text-[10px] text-white">打码后</span></button></div></article>)}</div> : <div className="mt-5 rounded-2xl bg-muted/55 px-6 py-12 text-center"><Images className="mx-auto h-7 w-7 text-muted-foreground" /><p className="mt-3 text-sm text-muted-foreground">当前案件暂无图片证据</p></div>}
      </section>
